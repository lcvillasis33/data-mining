...
Escribe la fecha actual (**NOTE-01**): 5 de junio de 2021
...
```{r}
NOMBRE_COMPLETO = "Luis Castillo Villasís"
DIGITOS_MATRICULA = 01379709
```
...
- Support del item {beer} (**NOTE-06**): 0.6
- Support del itemset {beer, nuts} (**NOTE-07**): 0.2
...
- support (mínimo) (**NOTE-12**): 0
- confidence (mínimo) (**NOTE-13**): 0.8
- items (máximo) (**NOTE-14**): 6
- tiempo de verificación de subsets (máximo) (**NOTE-15**): 0.00s
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**): que no hay nada asociado con la compra de pañales ya que casi siempre aparece en las listas
- Calcula el valor de la métrica support para la regla `{beer} => {nuts}` (**NOTE-20**): 0.2
- Calcula el valor de la métrica confidence para la regla `{beer} => {nuts}` (**NOTE-21**): 1.0
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): por los parámetros definidos para apriori
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): asignar una identificación a cada una de las 160 millones de sesiones incluidas en la base y poder dividirlas de aacuerdo a las acciones propias de cada usuario
- ¿Cuál es el propósito de `track id`? (**NOTE-24**): identificar de manera individual cada una de las 3.7 millones de canciones únicas encontradas en las 160 millones de sesiones
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): si, porque son variables independientes que podrían ayudar a definir y relacionar de mejor manera tanto el comportamiento de los usuarios como el desempeño de las canciones y de la plataforma digital
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): t_bacf06d3-9185-4183-84ea-ff0db51475ce con frecuencia de 875
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): 1579
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): es el número promedio de transacciones en un itemset
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**): los factores de confianza y soporte mínimo, no genera ninguna regla debido a que no identifica ningún itemset que cumpla con los valores dados en los parámetros
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? (**NOTE-30**): el número de reglas de asociación creadas puede ser o muy pequeño o muy grande y echar a perder el trabajo debido a la dificultad de correr dicho código
...
- Provee el enlace al video aquí: **NOTE-31**: https://www.youtube.com/watch?v=JIckWTi3wNU
